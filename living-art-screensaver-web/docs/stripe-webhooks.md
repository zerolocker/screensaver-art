...
2. **Repoint the app** — set `STRIPE_PRICE_ID` to the new id (`.env.local` for dev, Vercel Preview/Dev = test id, Vercel Production = live id) and redeploy. New checkouts now use it. **Also update the GitHub Actions `STRIPE_PRICE_ID` secret to the new *live* id** (`gh secret set STRIPE_PRICE_ID --body price_…`) — the price-drift CI guard reads it against the live `STRIPE_READONLY_KEY`, so skipping this fails the suite even when production is correct.
