...
2.  **Still Image Generation:**
    ...
    *   Generate a high-quality **4K** still with the **nano-banana-pro** skill — pass **`--size 4K`** (output is WebP by default, e.g. `--out gallery/<descriptive_name>_4k.webp`). Write the image prompt in line with `curation/PROMPT_GUIDANCE.md` (concrete medium/style/era/lighting; one clear subject) — **name the medium but not its physical condition**: no craquelure, aged/cracked varnish, visible brushwork or weave texture (see the "freshly made" rule — these render as distracting cracks, worst on dark scenes).
